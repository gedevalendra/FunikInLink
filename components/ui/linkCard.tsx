    ...
    <>
      ...
      {isAdmin && !isDummy && (
        <div 
          ...
          className={`flex items-center justify-center self-center p-2 rounded text-gray-400 cursor-grab hover:text-slate-700 hover:bg-slate-100 transition-colors shrink-0 touch-none select-none ${
      ...
      {/* IKON UTAMA LINK */}
      <div className="text-yellow-600 transition-colors pt-0.5 text-2xl shrink-0 select-none pointer-events-none">
        <i className={`bx ${link.icon}`}></i>
      </div>

      {/* INFORMASI KONTEN */}
      <div className="space-y-0.5 flex-1 min-w-0 pr-16 select-none">
        ...
        <a 
          ...
          className="inline-flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 font-medium pt-1 truncate max-w-full pointer-events-auto"
        >
        ...
        <div className="fixed inset-0 bg-black/40 z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-md p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form action={(formData) => { updateLink(formData); setIsEditing(false); }} className="space-y-4">
              ...
              <input name="title" defaultValue={link.title} placeholder="Judul" required className="w-full p-2.5 border border-gray-200 rounded-md text-sm focus:outline-none" />
              <input name="url" defaultValue={link.url} placeholder="URL Lengkap" required className="w-full p-2.5 border border-gray-200 rounded-md text-sm focus:outline-none" />
              <input name="description" defaultValue={link.description} placeholder="Deskripsi singkat (Opsional)" className="w-full p-2.5 border border-gray-200 rounded-md text-sm focus:outline-none" />
              
              <div className="pt-2">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase">Pilih Ikon</p>
                  <button type="button" onClick={() => setShowPicker(!showPicker)} className="text-xs text-yellow-600">
                ...
                {showPicker && (
                  <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 max-h-48 overflow-y-auto p-2 border bg-gray-50 rounded-md">
                    {BOXICONS.map(icon => (
                      <button key={icon} type="button" onClick={() => setSelectedIcon(icon)} className={`text-2xl p-1.5 rounded-md ${selectedIcon === icon ? 'bg-yellow-100 text-yellow-600 ring-2 ring-yellow-500' : 'text-gray-400'}`}>
              ...
              <div className="flex gap-3 mt-6">
                <button type="button" onClick={() => setIsEditing(false)} className="flex-1 py-2.5 bg-gray-100 rounded-md text-sm">Batal</button>
                <button type="submit" className="flex-1 py-2.5 bg-yellow-500 text-white rounded-md text-sm">Simpan</button>
              </div>
            </form>
          </div>
        </div>
